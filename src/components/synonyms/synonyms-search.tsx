...
export function SynonymsSearch() {
  ...
  const navigate = (word: string) => {
    ...
    setLoading(true)
    setShowSuggestions(false)
    router.push(`/synonyms/${encodeURIComponent(trimmed)}`)
  }
    ...
    <form onSubmit={handleSubmit} className="flex gap-2 w-full max-w-xl mx-auto">
      <div className="relative flex-1" ref={containerRef}>
        ...
        <input
          ...
          placeholder="Search any word — e.g. happy, brave, fast…"
      ...
      >
        {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Find Synonyms'}
      </button>
    </form>
  )
}
