...
export function HyponymsSearch() {
  ...
  const navigate = (word: string) => {
    ...
    setLoading(true)
    setShowSuggestions(false)
    router.push(`/types-of/${encodeURIComponent(trimmed)}`)
  }
    ...
    <form onSubmit={handleSubmit} className="flex gap-2 w-full max-w-xl mx-auto">
      <div className="relative flex-1" ref={containerRef}>
        ...
        <input
          ...
          placeholder="Enter a word — e.g. dog, fruit, vehicle, sport…"
      ...
      >
        {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Find Types'}
      </button>
    </form>
  )
}
