...
export function NounsForSearch() {
  ...
  const navigate = (word: string) => {
    ...
    setLoading(true)
    setShowSuggestions(false)
    router.push(`/nouns-for/${encodeURIComponent(trimmed)}`)
  }
    ...
    <form onSubmit={handleSubmit} className="flex gap-2 w-full max-w-xl mx-auto">
      <div className="relative flex-1" ref={containerRef}>
        ...
        <input
          ...
          placeholder="Enter an adjective — e.g. brave, dark, golden…"
      ...
      >
        {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Find Nouns'}
      </button>
    </form>
  )
}
