...
export function FrequencySearch() {
  ...
  const navigate = (word: string) => {
    ...
    setLoading(true)
    setShowSuggestions(false)
    router.push(`/word-frequency/${encodeURIComponent(trimmed)}`)
  }
    ...
    <form onSubmit={handleSubmit} className="flex gap-2 w-full max-w-xl mx-auto">
      <div className="relative flex-1" ref={containerRef}>
        ...
        <input
          ...
          placeholder="Enter any word — e.g. happy, ephemeral, bold…"
      ...
      >
        {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Check Frequency'}
      </button>
    </form>
  )
}
