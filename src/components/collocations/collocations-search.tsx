...
export function CollocationsSearch() {
  ...
  const navigate = (word: string) => {
    ...
    setLoading(true)
    setShowSuggestions(false)
    router.push(`/collocations/${encodeURIComponent(trimmed)}`)
  }
    ...
    <form onSubmit={handleSubmit} className="flex gap-2 w-full max-w-xl mx-auto">
      <div className="relative flex-1" ref={containerRef}>
        ...
        <input
          ...
          placeholder="Search any word — e.g. heavy, make, feel…"
      ...
      >
        {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Find Collocations'}
      </button>
    </form>
  )
}
